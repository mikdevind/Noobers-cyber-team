...
class Phase2 {
	...
	function R()
	{
		...
		$cos = (($this->n * array_sum($this->x2)) - (array_sum($this->x)*array_sum($this->x))) * (($this->n * array_sum($this->y2)) - (array_sum($this->y) * array_sum($this->y)));
		$tang = $sin / sqrt($cos);
		return round($tang, 4);
	}
...
function mean($data)
	{
		sort($data);
		return array_sum($data) / sizeof($data);
	}
?>